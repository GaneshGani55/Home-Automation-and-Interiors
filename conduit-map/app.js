...
const CONDUITS = [
 ...
 {ref:"P13",floor:"GF",service:"power_light",size:"25mm",status:"live",from:"DB",to:"Step lights + mid-flight wall light + server + landing",contents:"1.5sqmm L + N + E",note:"Step lights (3W each) on each riser left side."},
 ...
 {ref:"H7",floor:"GF",service:"power_heavy",size:"25mm",status:"live",from:"DB",to:"Fridge alcove socket at 300mm FFL",contents:"2.5sqmm L + N + E",note:"Confirm height after fridge model is picked."},
 ...
 {ref:"D1",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Foyer screen cavity (back wall, bottom-RIGHT)",contents:"1x indoor Cat6 (UTP)",note:"FLOOR route under screed (not ceiling). ~12m.",explain:"Cat6 (floor route under the screed) from the server to the foyer screen cavity. It powers (PoE) and feeds the Raspberry Pi behind the welcome monitor. Shares one floor chase with D2 + D3 - no separate break."},
 {ref:"D2",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Main-door outside face (Hikvision DS-KV6113-WPE1(C) video doorbell, PoE)",contents:"1x outdoor LSZH UV-rated Cat6 (PoE)",note:"Doorbell at 1450mm FFL, latch side. Shared floor route with D1+D3. Updated 2026-05-22: Hikvision DS-KV6113-WPE1(C) replaces the earlier Reolink TC541.",explain:"Outdoor Cat6 (PoE) to the video doorbell. ONE cable both powers the doorbell and carries its video back to the server for face detection - no power needed at the door."},
 {ref:"D3",floor:"GF",service:"data",size:"25mm",status:"provision",from:"Server",to:"Porch W wall (future face-detection camera, CAM-1)",contents:"PULL STRING ONLY - capped IP67 back-box, 1650mm FFL",note:"DROPPED from base install (doorbell handles face capture).",explain:"Spare conduit for a future dedicated face camera if the doorbell's face detection ever proves unreliable. Pull-string only now; a Cat6 can be drawn later - no wall break."},
 {ref:"D4",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Living Hall N wall, MIDDLE (overview camera, CAM-2)",contents:"1x outdoor Cat6 (PoE) - conduit route chosen on site",note:"Moved 2026-05-30: was porch ceiling NE corner; now mid of the Living Hall NORTH wall. Technician picks the conduit route.",explain:"Overview camera relocated to the middle of the Living Hall north (entrance-facing) wall - it watches the porch / main approach. Conduit route is left to the technician: pull from the server, or tap the nearest data run, whatever is cleanest on site."},
 {ref:"D5",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Staircase S wall, 1500mm FFL (Waveshare 10.1in panel)",contents:"1x HDMI 2.0 (3m) + 1x USB-A->micro-USB (3m) + existing Cat6 spare",note:"CUSTOM 280x195x80mm masonry cavity (not modular). Above existing 2M switch with ~200mm gap.",explain:"Two cables (HDMI + USB) from the Beelink in the server carry picture + touch straight to the staircase panel - no computer at the screen. The old Cat6 in this conduit is kept as a labelled spare."},
 {ref:"D6",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Dining E wall (Waveshare dining panel)",contents:"1x indoor Cat6 + 1x pull string",note:"~12m / 35-40ft. Pi-at-screen pattern.",explain:"Cat6 from the server to the dining wall panel. A small Pi behind the screen gets PoE power + data on this ONE cable and renders the Home Assistant dashboard. The interior designer will try to fit this screen into the cantilevered kitchen shelves, so box height/position is kept flexible (500mm slack)."},
 {ref:"D7",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"Dining ceiling JB (mono speaker)",contents:"1x indoor Cat6 (PoE-capable) + 1x pull string",note:"Pi + PAM8403 + 3in ceiling speaker. ~10m. This is the LOW-voltage speaker run via the server.",explain:"Cat6 from the server to a ceiling junction box above the dining table for a mono ceiling speaker. PoE-capable, so a small Pi/amp can sit at the ceiling - or the dining panel's Pi can drive it (see D11)."},
 {ref:"D9",floor:"GF",service:"data",size:"25mm x 3",status:"live",from:"Server",to:"Vertical riser -> FF -> Terrace (3 bundles)",contents:"3x LV-25: R-FF-1 (2x Cat6), R-FF-2, R-FF-3. CAM-3, CAM-5 and the Sintex run continue UP to terrace.",note:"Dedicated 150mm-wide chase channel up the staircase wall.",explain:"The vertical data spine. 3 grey conduits up the staircase wall carry ALL First-Floor and Terrace data - FF APs, the two study drops, the balcony + terrace cameras, and the Sintex tank sensor run all continue up here. See the Riser and Terrace tabs for what reaches the top."},
 {ref:"D10",floor:"GF",service:"data",size:"25mm",status:"live",from:"Server",to:"GF false-ceiling AP, central JB above dining/living junction (R-GF-1)",contents:"1x Cat6 UTP (PoE) + draw wire",note:"Ubiquiti UniFi U6-Lite or equiv. PoE only - no socket. Primary GF Wi-Fi 6.",explain:"Cat6 (PoE) from the server to the GF ceiling Wi-Fi AP above the dining/living junction. One cable powers the AP and backhauls it - no socket needed in the ceiling."},
 ...
 {ref:"L5",floor:"GF",service:"lv16",size:"16mm",status:"provision",from:"Server",to:"Main door frame TOP (concealed door-contact sensor)",contents:"PULL STRING ONLY",note:"Future provision - pull-string now.",explain:"For a FUTURE hidden magnetic door-contact sensor at the top of the main-door frame. It simply tells Home Assistant whether the door is OPEN or SHUT - two thin wires, one reed switch on the frame + a magnet on the door leaf. HA uses it for the 'Away' alarm and the welcome logic (e.g. only greet when the door actually opens). Just a pull-string today; a thin 2-core can be drawn later through THIS 16mm conduit, or piggy-backed on the adjacent doorbell conduit, so no wall-breaking later."},
 ...
 {ref:"W1",floor:"GF",service:"water",size:"20mm",status:"live",from:"Server",to:"Terrace SW Sintex JB (up the riser)",contents:"1x outdoor LSZH Cat6 (PoE)",note:"Up the staircase riser to the terrace, then embedded under terrace screed BEFORE tiling to the SW corner. See the Terrace tab.",explain:"Cat6 (data + 48V PoE) from the server, up the riser, across the terrace to the Sintex tank JB at the SW corner. It powers the tank's ESP32 and carries the ultrasonic water-level reading back to the server. Sensor run only - the float is separate (W2)."},
 ...
 {ref:"W3",floor:"GF",service:"water",size:"20mm",status:"live",from:"Server",to:"Sump JB (porch W wall, 300mm AGL)",contents:"1x outdoor LSZH Cat6 (PoE)",note:"Sleeve through the wall sloping outward. JB beside the manhole (cover hinges W).",explain:"Cat6 (data + 48V PoE) from the server to the sump JB on the porch. Powers the sump ESP32 and reads the pressure-probe water level. Sensor run only - the float is separate (W4)."},
 ...
 {ref:"W5",floor:"GF",service:"water",size:"20mm",status:"provision",from:"Server",to:"DB starter cupboard (future backup link)",contents:"PULL STRING ONLY",note:"Both ends capped + labelled WATER AUTOMATION - FUTURE Cat6.",explain:"Spare conduit between the server and the cupboard. If the Wi-Fi link to the motor controller ever proves unreliable, a Cat6 can be pulled here later for a wired ESP32 motor controller - no wall break."},
...
const CO = {
 ...
 H1:{o:HUB.DB,d:[200,914]}, H2:{o:HUB.DB,d:[928,880]}, H3:{o:HUB.DB,d:[650,985]}, H4:{o:HUB.DB,d:[112,150]},
 ...
 D11:{o:[80,470],d:[210,560]}, D12:{o:HUB.NICHE,d:[928,902]},
...
/* LOCKED 2026-05-30 — see automation-iot/mmwave-presence-sensors.md and the 3 PDFs in assets/pdf/. */
const MMWAVE = [
 {ref:"M1",floor:"GF",room:"GF Master Bedroom",mount:"Ceiling (centre)",powerShort:"Sonoff SNZB-06PR2 · buck 230→5V in switch cavity",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Ceiling mount: HLK-PM01 buck (230→5V) inside the MBR switch cavity. Thin 2-core 0.75mm² flex copper through 20mm PVC conduit up to the ceiling sensor cavity. USB-C plug at sensor end.",
  mountNote:"Ceiling centre of the master bedroom, clear of the wardrobe path. False ceiling planned — cavity 86×86×40mm pre-plaster.",xy:[800,300]},
 {ref:"M2",floor:"GF",room:"GF Common Bathroom",mount:"Wall on existing JB",powerShort:"Sonoff SNZB-06PR2 · Method 1 (JB Mount)",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Method 1 (JB Mount): HLK-PM01 buck converter sits INSIDE the existing circular metal JB at 7ft FFL on the N wall middle. 230V tapped from L1 mirror chase via Wago 221 lever connectors. USB-C cable emerges through 8mm rubber grommet in JB cover; sensor mounts magnetically on the cover face. Zero visible cable.",
  mountNote:"N wall middle, 7ft FFL — on the existing circular JB extended from the hand-basin lighting chase. Sensor cone covers both dry zone AND shower zone through the glass partition. Firmly outside IP Zone 1/2.",xy:[645,500]},
 {ref:"M3",floor:"GF",room:"Dining / Living mid-ceiling",mount:"Ceiling (multi-zone)",powerShort:"Aqara FP2 · buck 230→5V (8m run, 1.0mm² flex)",
  sensor:"Aqara FP2 (24GHz mmWave, WiFi+Matter, up to 30 zones, built-in lux)",
  power:"Ceiling mount: HLK-PM01 buck inside nearest switch cavity. 2-core 1.0mm² flex copper (NOT 0.75 — bumped up to handle voltage drop over ~8m run). USB-C at sensor end. Multi-zone Aqara FP2 — zones drawn in Aqara app (sofa / dining table / walkway). Exposed to HA via Matter.",
  mountNote:"Ceiling mid-point between Dining and Living, near the GF ceiling-AP junction box (D10). 25mm conduit with 1× Cat6 + 2× pull-strings for future ceiling speaker/AP/sensor.",xy:[430,655]},
 {ref:"M4",floor:"FF",room:"FF Bedroom 2 (W, balcony side)",mount:"Ceiling (centre)",powerShort:"Sonoff SNZB-06PR2 · buck in switch cavity",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Ceiling mount: HLK-PM01 buck inside BR2 switch cavity. 2-core 0.75mm² flex copper through 20mm conduit up to ceiling cavity (false ceiling present — D11 cove circuit confirms ceiling access).",
  mountNote:"Ceiling centre of the west/balcony bedroom. False ceiling planned (cove pocket 100×75mm).",xy:[705,430]},
 {ref:"M5",floor:"FF",room:"FF Bedroom 1 (E side)",mount:"Wall at 7ft FFL",powerShort:"Sonoff SNZB-06PR2 · Method E (buck in 2M GI box)",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Method E: 2M GI MS back box at 2100mm FFL on the wall facing the bed. HLK-PM01 buck inside the box, 230V via Wago 221 connectors. Short USB-C cable exits through hole in blank Schneider plate. Sensor on supplied bracket directly above the plate.",
  mountNote:"Wall mount at 7ft FFL — NO false ceiling planned (slab cast, direct paint). Wall position: facing the bed for clear sight line.",xy:[265,430]},
 {ref:"M6",floor:"FF",room:"FF Toilet 2 (W bedroom's bath)",mount:"Ceiling or Wall (TBD)",powerShort:"Sonoff SNZB-06PR2 · Method 1 / Method E",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Method 1 if ceiling access verified on site (HLK-PM01 inside ceiling JB). Otherwise Method E (2M GI box at 7ft on wall). Slab cast — verify ceiling chase access during plaster prep.",
  mountNote:"Dry zone, clear of the shower. Final mount choice on site by electrician.",xy:[675,150]},
 {ref:"M7",floor:"FF",room:"FF Toilet 1 (E bedroom's bath)",mount:"Wall at 7ft FFL",powerShort:"Sonoff SNZB-06PR2 · Method E (buck in 2M GI box)",
  sensor:"Sonoff SNZB-06PR2 (24GHz mmWave + PIR fusion, Zigbee 3.0)",
  power:"Method E: 2M GI MS back box at 2100mm FFL on the wall facing the entry door. HLK-PM01 buck inside box, sensor on bracket above.",
  mountNote:"Wall mount at 7ft FFL — NO false ceiling, slab cast. Position on entry-facing wall, dry zone, outside shower IP Zone 1/2.",xy:[325,150]}
];

/* mmWave reference PDFs — every sensor popup links to all 3 */
const MMWAVE_PDFS = [
 {file:"MMWAVE_SENSOR_MASTER.pdf", label:"Master reference (start here)"},
 {file:"SENSOR_MOUNTING_METHODS.pdf", label:"All mount methods (overview)"},
 {file:"JB_INSTALL_AND_NEW_APPROACHES.pdf", label:"JB install step-by-step + Methods D/E"}
];
...
const GUIDES = {
 "JB-SUMP":{
  ...
  power:"PoE only. One Cat6 (C-Sump-1 / W3) from the server carries data + 48V. NO 230V at this box. The PoE splitter drops 48V → 5V to the ESP32; the ESP32's 5V rail feeds the sensor.",
 ...
 "JB-SINTEX":{
  ...
  power:"PoE only, via C-Sintex-1 (W1) Cat6 from the server. No 230V at the JB.",
 ...
 "NICHE-SERVER":{
  title:"Server (the brain)", sub:"Staircase · HA + Frigate + PoE backbone", glyph:"HA", svc:"data",
  badge:"Pull cable now",
  ...
  mount:"Server shelving; Cat6 patch panel + speaker terminal strip at 700mm FFL on the side wall. This is the origin hub for all D-series (data), W1/W3 (water sensors) and the FF riser bundle.",
 ...
 "FOYER-CAVITY":{
  ...
  power:"Cavity has 2× 5A sockets on the BACK wall (C-DB-Cavity-Power / H1, from the DB on the E wall). The Pi is PoE-powered: the cavity Cat6 (D1, floor route from the server) → PoE splitter → 5V to the Pi. Halo 24V from the driver above the false ceiling.",
 ...
 "AP-GF":{
  ...
  badge:"Pull cable now",
  inside:["Ubiquiti UniFi U6-Lite (or equivalent) Wi-Fi 6 AP","Fed by 1× Cat6 (R-GF-1 / D10) from the server"],
  power:"PoE only — the AP draws ~12W over the Cat6 from the server PoE switch. No socket needed.",
  ...
  steps:["Run R-GF-1 (D10) from the server, up the server wall, east across the GF false-ceiling void.","Terminate at a capped ceiling JB at the central dining/living position.","After ceiling work, clip the AP to the JB and patch its Cat6 to a PoE port in the server."],
 ...
 "AP-FF":{
  ...
  steps:["Carry R-FF-1 up the staircase riser, then east to the central FF Living ceiling.","Terminate 2× Cat6 at a ceiling JB.","Mount the AP after ceiling work; patch one Cat6 to a server PoE port (keep the 2nd as spare)."],
 ...
 "CAM-DOORBELL":{
  ...
  steps:["Recess the small GI box on the outside N wall at 1450mm.","Pull D2 outdoor Cat6 (floor route) to the box; weatherproof the entry with silicone.","Mount the doorbell bracket over the box; patch its Cat6 to a server PoE port.","Add it to Frigate; train CompreFace on family faces."],
 ...
 "CAM-N":{
  ...
  steps:["Pick the conduit route on site (from the server, or tap the nearest data run) — left to the technician.","Land a Cat6 at a flush / IP-rated box at the middle of the Living N wall.","Mount the camera, aim out over the porch / approach; patch to a server PoE port."],
 ...
 "WAVESHARE-STAIR":{
  ...
  power:"Beelink-direct — no Pi, no PoE. HDMI + USB run through the existing C-Server-Waveshare conduit (D5); 5V to the screen comes over the USB from the Beelink. The existing Cat6 stays as a labelled SPARE.",
  ...
  steps:["Mason chases the 280×195×80mm cavity (before plaster, 2026-05-30).","Conduit enters the cavity bottom via a ~30mm hole; smooth-finish or black-ply liner.","Tape HDMI + USB to the existing Cat6 and pull all three; leave 200mm service loops.","Server end: HDMI → Beelink HDMI-2, USB → Beelink USB-A.","Carpenter builds the bezel; mount the screen; Beelink runs a Chromium kiosk on HDMI-2."],
 ...
 "WAVESHARE-DINING":{
  ...
  inside:["Today: 1× Cat6 + pull-string + capped 3\"×3\"×2.5\" back box (C-Server-Dining / D6)","Future: Waveshare 10.1\" + a small Pi behind the screen (Pi-at-screen)"],
...
function drawRiser(g){
  const L=140,R=620,levels=[
    {y:120,t:"TERRACE"},{y:320,t:"FIRST FLOOR"},{y:620,t:"GF SLAB"},{y:920,t:"GROUND (Server)"}
  ];
  levels.forEach(lv=>{
  ...
  g.appendChild(txt((L+R)/2,108,"STAIRCASE RISER CHASE  (GF Server → FF → Terrace)","rsub"));
...
function drawTerrace(g){
  ...
  // W2 float leaves the tank and runs ANTICLOCKWISE along the parapet boundary,
  // descending at the NE corner (above the GF DB cupboard) — matches the GF view
  const w2=el("path",{d:"M745,300 L745,182 L162,182 L162,892",fill:"none",stroke:SVC.water.color,
    "stroke-width":4,"stroke-dasharray":"7,5","stroke-linecap":"round","stroke-linejoin":"round"});
  g.appendChild(w2);
  g.appendChild(el("path",{d:"M155,888 L169,888 L162,903 Z",fill:SVC.water.color})); // descent arrow
  g.appendChild(txt(745,395,"W1 ↑","rsub"));
  g.appendChild(txt(300,176,"W2 ↺ along parapet","rsub")).setAttribute("text-anchor","start");
  g.appendChild(txt(180,920,"W2 ↓ down to DB cupboard","rsub")).setAttribute("text-anchor","start");
...
function openMmwave(ref){
  ...
  renderGuide({
    ...
    badge:"sensor — LOCKED 2026-05-30",
    inside:[m.sensor||"mmWave presence sensor","HLK-PM01 buck converter (230V AC → 5V DC, ₹200, industrial PCB)","Wago 221-413 lever connectors for 230V tap","Reports to Home Assistant via Zigbee2MQTT (or WiFi+Matter for FP2)"],
    power:m.power,
    mount:m.mountNote,
    note:"Sensor LOCKED 2026-05-30. See the 3 PDFs at the bottom of this card for full install instructions including step-by-step images, mount methods, wiring diagrams, and BOM. HA + Z2M handle pairing; in direct mode the room's wall rocker still works even when HA is down.",
    steps:["Cut the cavity / box at the location described above.","Run 20mm PVC conduit from sensor location to the room's switch cavity, with nylon pull-string inside.","Pull 2-core 0.75mm² flex copper through the conduit.","In the switch cavity (or JB / 2M GI box), wire HLK-PM01 to 230V via Wago 221 connectors.","Solder/connect HLK-PM01 5V output to a sacrificed USB-C cable. Heat-shrink all joints.","Plug USB-C into the sensor; mount sensor magnetically (JB cover) or on its supplied bracket.","Pair to Home Assistant (Z2M for Sonoff, Matter for Aqara FP2)."],
    pdfs:MMWAVE_PDFS
  });
}
...
function buildGuideBody(G){
  ...
  if(G.pdfs && G.pdfs.length && !window.__OFFLINE){
    h+=`<div class="gsec"><h4>Reference PDFs</h4><div style="display:flex;flex-direction:column;gap:6px">`+
       G.pdfs.map(p=>`<a class="gpdf" href="${assetUrl('assets/pdf/'+p.file)}" target="_blank" rel="noopener">${p.label}</a>`).join("")+
       `</div></div>`;
  }
  return h;
}
