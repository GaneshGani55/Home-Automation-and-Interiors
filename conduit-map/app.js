...
/* Filter active on load (automation-first). */
const DEFAULT_ACTIVE = ["data","lv16","water","presence"];
...
/* ============================================================
   ...
   ============================================================ */
const HUB = { DB:[88,935], NICHE:[820,805], DBCUP:[88,978], RISER:[800,705] };
...
const CO = {
 ...
 H1:{o:HUB.DB,d:[195,955]}, H2:{o:HUB.DB,d:[928,800]}, H3:{o:HUB.DB,d:[650,985]}, H4:{o:HUB.DB,d:[112,150]},
 ...
 D1:{o:HUB.NICHE,d:[205,955]}, D2:{o:HUB.NICHE,d:[205,1010]}, D3:{o:HUB.NICHE,d:[235,1038]},
 ...
 W1:{o:HUB.NICHE,d:[845,660]}, W2:{o:HUB.DBCUP,d:[835,715]}, W3:{o:HUB.NICHE,d:[505,1042]},
...
const DEVICES = [
 ...
 {id:"NICHE-SERVER", floor:"GF",   svc:"data",  label:"Server niche",   xy:[820,772]},
 {id:"FOYER-CAVITY", floor:"GF",   svc:"data",  label:"Foyer screen",   xy:[200,952]},
 ...
 {id:"SMART-SWITCH", floor:"GF",   svc:"power_light", label:"Smart switch box", xy:[215,912]},
...
function drawPlan(floor,g){
  ...
  if(floor==="GF") drawStair(g,710,610,860,830,14,"UP",true);
  if(floor==="FF"){ drawStair(g,710,660,870,820,12,"UP TERRACE",false);
    g.appendChild(txt(790,650,"to terrace","rsub")); }
...
function drawHubs(floor,g){
  const list = floor==="GF"
    ? [["NICHE",HUB.NICHE,"#FFE650","NICHE",90,26],
...
function drawConduits(floor,g){
  ...
  list.forEach((c,i)=>{
    ...
    const dpath=routeD(co.o,co.d,jit);
...
function buildChips(){
  ...
  c.appendChild(auto); c.appendChild(all);
  SVC_ORDER.forEach(k=>{
    const b=mk(`<span class="sw" style="background:${SVC[k].color}"></span>${SVC[k].label}`);
    b.dataset.svc=k;
    b.onclick=()=>{ active=(active.size===1&&active.has(k))?new Set(DEFAULT_ACTIVE):new Set([k]); syncChips(); apply(); };
    c.appendChild(b);
  });
  const comp=mk(`<span class="sw" style="background:#2F2C28"></span>Components`); comp.dataset.key="components";
  comp.onclick=()=>{ active=(active.size===1&&active.has("components"))?new Set(DEFAULT_ACTIVE):new Set(["components"]); syncChips(); apply(); };
  c.appendChild(comp);
  syncChips();
}
function syncChips(){
...
function apply(){
  ...
  document.querySelectorAll(`#view-${floor} .dev`).forEach(g=>{
    ...
    const passSvc = active.size===0 || active.has("components") || (d && active.has(d.svc));
