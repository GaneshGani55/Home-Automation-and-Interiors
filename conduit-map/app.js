...
function drawPlan(floor,g){
  ...
  if(floor==="GF") drawStair(g,560,610,860,830,22,"UP",true);
  if(floor==="FF"){ drawStair(g,640,660,880,820,12,"UP TERRACE",false);
...
function buildChips(){
  ...
  SVC_ORDER.forEach(k=>{
    ...
    b.className="chip"; b.dataset.svc=k;
    b.innerHTML=`<span class="sw" style="background:${SVC[k].color}"></span>${SVC[k].label}`;
    b.onclick=()=>{active.has(k)?active.delete(k):active.add(k); syncChips(); apply();};
    c.appendChild(b);
  });
  syncChips();
}
function syncChips(){
  document.querySelectorAll(".chip").forEach(b=>{
    const on=active.has(b.dataset.svc);
    b.classList.toggle("on",on);
    b.classList.toggle("off",active.size>0&&!on);
  });
}
...
function clampVB(){
  const minW=BASE.w*0.32, maxW=BASE.w;
...
function zoomAt(vx,vy,f){const nw=Math.max(BASE.w*0.32,Math.min(BASE.w,vb.w*f));
